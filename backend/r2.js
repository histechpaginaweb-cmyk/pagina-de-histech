...
// ─────────────────────────────────────────────────────────────
// CASOS DE ÉXITO (editables desde el admin). Datos en cases.json (R2).
// Cada caso: slug, title, category, icon, challenge, solution, result,
// metric, metricLabel, tech[], relatedServices[]. Estructura Reto/Solución/
// Resultado. El frontend lo consume; si el backend no responde, usa el seed.
// ─────────────────────────────────────────────────────────────
const CASES_KEY = "cases.json";
...
/** Lee los casos de éxito desde R2. Si no existen, los siembra. */
async function readCases() {
  if (!client) return CASES_SEED.map((c) => ({ ...c }));
  try {
    const res = await client.send(
      new GetObjectCommand({ Bucket: R2_BUCKET, Key: CASES_KEY }),
    );
    const arr = JSON.parse(await streamToString(res.Body));
    if (!Array.isArray(arr)) throw new Error("cases.json inválido");
    return arr;
  } catch (err) {
    const notFound =
      err?.name === "NoSuchKey" ||
      ...
      err?.$metadata?.httpStatusCode === 404;
    if (notFound) {
      const seed = CASES_SEED.map((c) => ({ ...c }));
      await writeCases(seed);
      return seed;
    }
    throw err;
  }
...
/** Sobrescribe cases.json en R2. */
async function writeCases(items) {
  if (!client) throw new Error("R2 no está configurado en el servidor");
  await client.send(
    new PutObjectCommand({
      Bucket: R2_BUCKET,
